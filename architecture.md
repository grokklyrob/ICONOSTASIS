...
| AMD-30 | §18, §19 | BYOK-cloud-first: M2 demo gate names a cloud provider (OpenRouter reference path), not a local Ollama run. §4.2 local-inference capability retained — `openai-compat` is generic, so a user-supplied localhost `baseUrl` still works. |
...
- **M2 — Oracle (split, ~3–4 wks total):**
  - **M2a — Boundary:** Key Vault, SecretRef fetch boundary, taint gate, spend ceiling plumbing, Provider Registry shell, arming UI hooks, first live adapter path via `openai-compat` (OpenRouter or any OpenAI-compatible endpoint, local servers included).
  ...
  *Demo: signals→prompt→image→texture and generated voice, live, on the user’s own key via a cloud BYOK provider (OpenRouter is the reference path); demonstrate armed/disarmed Perform behavior and hard spend stop. (AMD-30 — local inference remains supported via `openai-compat` but no longer gates the milestone.)*
...
- **Adapter contract tests:** each adapter against a mock server implementing its wire format; `openai-compat` additionally smoke-tested against a real endpoint in CI-optional mode, configured by env (`ICONOSTASIS_SMOKE_BASE_URL` / `_MODEL` / `_API_KEY`) so any OpenAI-compatible provider — cloud or local — can serve as the target.
